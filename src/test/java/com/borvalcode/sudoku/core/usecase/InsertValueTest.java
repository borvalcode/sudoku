...
public class InsertValueTest {
  ...
  void should_go_ok_if_validates() {
    ...
    when(this.verifyInsertValue.execute(insert)).thenReturn(Either.right(Boolean.TRUE));
  ...
  void should_return_error_when_sudoku_complete() {
    Insert insert =
        Insert.builder().sudoku(aCompleteSudoku()).coordinate(Coordinate.of(1, 3)).value(8).build();
